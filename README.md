...
Only x86_64 is supported. Tested on Nobara/Fedora 43 and similar `dnf`-based
distros; `apt-get`-based distros (Debian/Ubuntu) should also work.

For building ([build_setup.py](./build_setup.py)):

- `build_setup.py` will prompt to install `clang` and ALSA dev headers and will
  automatically download FFmpeg 8 pre-built shared libraries from
  [BtbN/FFmpeg-Builds](https://github.com/BtbN/FFmpeg-Builds) into `./ffmpeg/`.

For packaging ([build_package.py](./build_package.py)):

- `patchelf` is required (the script will offer to install it via `dnf` or
  `apt-get`).
